...
$string['allowed_acara_ids']        = 'Allowed ACARA IDs';
$string['allowed_acara_ids_desc']   = 'ACARA IDs this site may provision for, one per line or comma separated. A provisioning call naming an ACARA ID outside this list is rejected. Leave blank to accept any ACARA ID. If blank, the single Default ACARA ID below is used as the list instead.';
$string['validate_isbn']            = 'Validate ISBNs';
$string['validate_isbn_desc']       = 'Reject subscriptions whose ISBN fails check-digit validation. This applies only when no product catalogue has been loaded — when products exist, the catalogue is authoritative and unknown ISBNs are rejected regardless of this setting.';
$string['sso_autocreate']           = 'Create Moodle accounts on first SSO login';
$string['sso_autocreate_desc']      = 'When a user arrives via Campion SSO with a verified token but has no Moodle account, create one automatically. The account has no usable password and is reachable only through Campion SSO. Leave disabled if Moodle accounts are provisioned by your own enrolment process, in which case SSO for an unknown user fails with an error instead.';
